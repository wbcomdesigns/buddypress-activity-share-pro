...
set -u
F="${1:?token css file}"; P="${2:-bp-share}"
# Surface token vocabulary varies per plugin: polls uses --*-card-bg, share
# uses --*-surface; both also expose --*-bg. Match any of them.
SURF="(surface|card-bg|bg)"
...
echo "$db" | grep -qE -- "--${P}-${SURF}\s*:" && ok "dark flips a surface token" || no "dark flips a surface token"
...
echo "$db" | grep -E -- "--${P}-${SURF}\s*:" | grep -q "#" && ok "dark surface uses an explicit value" || no "dark surface uses an explicit value"
# brace balance
