...
|------|------|-------------|-------|
| 2026-06-04 | Refactor | 2.3.0 admin UX revamp (pre-testing wrap): legacy `wbcom-wrapper` retired for the shared card-panel shell under the `wbcomplugins` hub, first-run onboarding (`bpas_onboarding_complete` site option, shows once then never again), P0 dead-setting wiring (settings that rendered but never applied are now read at the frontend), services/icon-style dropdown modernization, and the blue-on-blue primary-button contrast fix. UX-only — all option keys/scopes, AJAX actions, nonces, meta, cron, and hooks preserved. | admin/class-bpas-admin-panel.php, admin/views/*, admin/css*/*, admin/js/*, includes/class-buddypress-share.php, public/class-buddypress-share-public.php, includes/post-types/class-bp-share-post-type-frontend.php |
